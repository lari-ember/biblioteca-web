...
def index():
    ...
    Features:
    - Exibe livros em destaque para usuários autenticados
    - Fornece conteúdo de onboarding para novos visitantes
    - Coleta métricas básicas de engajamento

    Returns:
        Renderiza o template index.html com contexto dinâmico.
    """
    ...
    try:
        ...
        '''context = {
            ...
            "accessibility_features": get_accessibility_settings()
        }'''

        # Monitoramento
        #log_activity(f"Homepage access - User: {current_user.id if current_user.is_authenticated else 'guest'}")
...
def get_featured_books():
    """Obtém livros em destaque com fallback estratégico"""
    if current_user.is_authenticated:
        return Book.query.filter_by(user_id=current_user.id).order_by(Book.created_at.desc()).limit(4).all()
    return Book.default_featured_books()


def generate_seo_metadata():
    """Gera metadados dinâmicos para SEO"""
    base_desc = "Digital library management system with AI-powered features"
    return {
        "title": f"Amber Archively | {current_user.username}'s Library" if current_user.is_authenticated else "Amber Archively",
        "description": f"{current_user.username}'s personal {base_desc}" if current_user else base_desc,
